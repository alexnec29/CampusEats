...
const Menu: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filtered = menuItems.filter(item => {
      if (!item.menuItemAllergens || item.menuItemAllergens.length === 0) {
        return true; // Show items with no allergens
      }
      
      // Exclude items that contain the filtered allergen
      return !item.menuItemAllergens.some(mia => 
        mia.allergen.name.toLowerCase().includes(allergenFilter.toLowerCase())
      );
    });

    setFilteredItems(filtered);
  }, [allergenFilter, menuItems]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-6 page-transition">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 bg-white p-4 rounded-xl shadow-md">
          <label htmlFor="allergen-filter" className="block text-sm font-medium text-gray-700 mb-2">
            🔍 Exclude Allergen (e.g., "nuts", "dairy", "gluten")
          </label>
          <input
            id="allergen-filter"
            type="text"
            ...
            placeholder="Type allergen name to exclude..."
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {allergenFilter && (
            <p className="mt-2 text-sm text-gray-600">
              Showing items without <strong>{allergenFilter}</strong> ({filteredItems.length} items)
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div key={item.id} className="bg-white rounded-2xl shadow-lg overflow-hidden transform transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
                ...
                <div className="p-6">
                    ...
                    {/* Allergen badges */}
                    {item.menuItemAllergens && item.menuItemAllergens.length > 0 && (
                        <div className="mb-4 flex flex-wrap gap-2">
                            {item.menuItemAllergens.map((mia, index) => (
                                <span 
                                    key={index}
                                    className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-300"
                                    title="Allergen"
                                >
                                    ⚠️ {mia.allergen.name}
                                </span>
