...
using CampusEats.Api.Features.Order.ApplyLoyaltyPoints;
using MediatR;
...
public static class OrderEndpoints
{
    public static void MapOrderEndpoints(this WebApplication app)
    {
        ...
        orders.MapPost("/{orderId}/apply-loyalty-points", async (int orderId, ApplyLoyaltyPointsRequest request, HttpContext httpContext, IMediator mediator) =>
        {
            var userIdString = httpContext.User.FindFirst("/id")?.Value;
            if (string.IsNullOrEmpty(userIdString) || !Guid.TryParse(userIdString, out var userId))
            {
